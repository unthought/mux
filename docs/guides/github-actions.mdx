...
This example shows a periodic cleanup workflow that identifies low-risk cleanup opportunities and maintains a refactor PR with improvements.
...
The workflow uses a dedicated repository secret (`AUTO_CLEANUP_ANTHROPIC_API_KEY`) so auto-cleanup spend stays isolated from other jobs, while the runtime environment still uses the standard `ANTHROPIC_API_KEY` name.
